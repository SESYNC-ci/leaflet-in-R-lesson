...
```{r leaflet_layers, title = "worksheet-1.R", eval = TRUE}
leaflet() %>%
  addTiles(group = "OSM") %>%
...
```{r leaflet_wms, title = "worksheet-1.R", eval = TRUE}
leaflet() %>%
  addTiles() %>%
...
```{r leaflet_topo, title = "worksheet-1.R", eval = TRUE}
...
```{r leaflet_mm, title = "worksheet-1.R", eval = TRUE}
leaflet() %>% 
  setView(lng = -111.846061, lat = 36.115847, zoom = 12) %>%
  addWMSTiles(nhd_wms_url, layers = "0") %>%
...
```{r leaflet_extras, title = "worksheet-1.R", eval = TRUE}
leaflet() %>% 
  setView(lng = -111.846061, lat = 36.115847, zoom = 12) %>%
  addWMSTiles(nhd_wms_url, layers = "0") %>%
  addMiniMap(zoomLevelOffset = -4) %>%
