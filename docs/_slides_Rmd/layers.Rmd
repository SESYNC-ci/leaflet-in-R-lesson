...
```{r leaflet_layers, title = "{{ site.handouts[0] }}"}
leaflet() %>%
  addTiles(group = "OSM") %>%
...
```{r leaflet_wms, title = "{{ site.handouts[0] }}"}
leaflet() %>%
  addTiles() %>%
...
```{r leaflet_topo, title = "{{ site.handouts[0] }}"}
...
```{r leaflet_mm, title = "{{ site.handouts[0] }}"}
leaflet() %>% 
  setView(lng = -111.846061, lat = 36.115847, zoom = 12) %>%
  addWMSTiles(nhd_wms_url, layers = "0") %>%
...
```{r leaflet_extras, title = "{{ site.handouts[0] }}"}
leaflet() %>% 
  setView(lng = -111.846061, lat = 36.115847, zoom = 12) %>%
  addWMSTiles(nhd_wms_url, layers = "0") %>%
  addMiniMap(zoomLevelOffset = -4) %>%
...
```{r leaflet_findme, title = "{{ site.handouts[0] }}"}
leaflet() %>%
  addTiles() %>% 
